...
function map5d( arr, fcn, thisArg ) {
	...
	if ( !isArrayLikeObject( arr ) ) {
		throw new TypeError( format( '0kg2a', arr ) );
	}
	if ( !isFunction( fcn ) ) {
		throw new TypeError( format( '0kg2S', fcn ) );
	}
	S0 = arr.length;
	out = [];
	for ( i0 = 0; i0 < S0; i0++ ) {
		...
		if ( !isArrayLikeObject( a1 ) ) { // note: cannot support the more general "collections" here (which includes typed arrays having more than 2^32-1 elements), as the output array is limited to 2^32-1 elements; thus, we opt for the lowest common denominator: generic arrays
			throw new TypeError( format( '0kgCE', i0, a1 ) );
		}
		S1 = a1.length;
		t1 = [];
		for ( i1 = 0; i1 < S1; i1++ ) {
			...
			if ( !isArrayLikeObject( a2 ) ) {
				throw new TypeError( format( '0kgCF', i0, i1, a2 ) );
			}
			S2 = a2.length;
			t2 = [];
			for ( i2 = 0; i2 < S2; i2++ ) {
				...
				if ( !isArrayLikeObject( a3 ) ) {
					throw new TypeError( format( '0kgCG', i0, i1, i2, a3 ) );
				}
				S3 = a3.length;
				t3 = [];
				for ( i3 = 0; i3 < S3; i3++ ) {
					...
					if ( !isArrayLikeObject( a4 ) ) {
						throw new TypeError( format( '0kgCH', i0, i1, i2, i3, a4 ) );
					}
					S4 = a4.length;
					t4 = [];
					for ( i4 = 0; i4 < S4; i4++ ) {
